...
def contrastive_loss_column(y_true: torch.Tensor,
    ...
    total_loss = contrastive_loss + beta * entropy_loss
...
def mimvc_loss(
    ...
    w = [args.ae_weight, args.dg_weight, args.contrastive_weight_column]
    loss = []
    for v in range(len(x)):
        # loss_v = w[0] * losses_sic[v] + w[1] * losses_rci[v] + w[2] * losses_cca_row[v] + w[3] * losses_cca_column[v]
        loss_v = w[0] * losses_sic[v] + w[1] * losses_rci[v] + w[2] * losses_cca_column[v]
        loss.append(loss_v)
    return loss
